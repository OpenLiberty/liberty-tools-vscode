...
describe("addProjectsToTheDashBoard", () => {
    ...
    afterEach(() => {
        ...
        sinon.restore();
        // Clear singleton so tests don't bleed into each other.
        ProjectRegistry.setInstance(undefined as any);
    });
    ...
    [
        ...
        it(`shows the expected message and refreshes the dashboard when addUserSelectedPath returns ${result}`, async () => {
            ...
            // Wire the registry singleton so addProjectsToTheDashBoard can call it.
            const fakeRegistry = { addUserSelectedPath: addUserSelectedPathStub } as any;
            ProjectRegistry.setInstance(fakeRegistry);

            const projectProvider = { refresh: refreshStub } as any;

            await addProjectsToTheDashBoard(projectProvider, "/my/project");

            assert.equal(addUserSelectedPathStub.calledOnceWithExactly("/my/project"), true);
